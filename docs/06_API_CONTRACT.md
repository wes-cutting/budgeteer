...
- **Calendar dates are caller-local (EH8, 04_DOMAIN_MODEL §6):** the server never derives a
  user-facing "today"/"this month". Every date/month parameter that used to default
  server-side is **required** — `occurredOn`, `reconciledOn`, `openedOn`, the register's
  `from`/`to`, budget-vs-actual's `month`, and the `today` of the recurring reads/writes and
  the forecast. The bundled web client derives them from the browser's local clock
...
- **Input:** `{ name: string, kind: AccountKind, startingBalance?: string = "0",
  openedOn: "YYYY-MM-DD" }` — `openedOn` is the caller-local date the opening row lands on (EH8).
- **Output:** `201 { "account": AccountView }` (`balanceCents` = the opening balance).
- **Errors:** `400` (empty name, unknown kind, unparseable amount, bad/missing `openedOn`); `409` (duplicate name).
...
- **`POST /accounts/:accountId/transactions`** — create a transaction + its allocations.
  ...
  Errors: `400` (amount ≤ 0, bad/missing date, over-allocation, net direction-flip, unknown/archived envelope); `404` (account).
- **`GET /accounts/:accountId/transactions`** — the account register (newest-first).
  Query: `?from=YYYY-MM-DD&to=YYYY-MM-DD` (inclusive date window over `occurredOn`), **required** —
  the caller supplies its local month; the server derives no default window (EH8; the R8
  current-month default is the web client's, from `localMonthRange()`). `opening` rows are
  **always** included regardless of the window, so the register keeps its balance anchor.
  Payee/memo search is **client-side** over the windowed rows. →
  `200 { transactions: TransactionView[] }`; `400` if `from`/`to` is missing or not
...
- **`POST /transfers`** — create a transfer. Input: `{ fromAccountId, toAccountId,
  ...
  Errors: `400` (amount ≤ 0, same account, archived account, bad/missing date); `404` (account missing).
...
- **`POST /envelope-transfers`** — create a reallocation. Input: `{ fromEnvelopeId, toEnvelopeId,
  ...
  Errors: `400` (amount ≤ 0, same envelope, **archived destination**, bad/missing date); `404` (envelope
...
- **`POST /accounts/:accountId/reconciliations`** `{ statementBalance: string, reconciledOn:
  ...
  amount, bad/missing date); `404` (account missing).
...
(positive), refund }`. `dueCount` = occurrences on/before the **caller's** `today` not yet posted
(EH8 — due-ness is a user-local calendar judgment, so every read/write that involves it takes
`today` from the caller).

- **`GET /recurring?today=YYYY-MM-DD`** (`today` **required**) → `200 { recurring: RecurringView[] }`;
  `400` if `today` is missing/malformed.
- **`POST /recurring`** `{ accountId, kind: "deposit"|"withdrawal", amount: string, payee?, memo?,
  frequency, anchorOn: "YYYY-MM-DD", today: "YYYY-MM-DD", lines: [{ envelopeId, amount, refund? }] }`
  → `201 { recurring }` (`today` anchors the returned view's `dueCount`).
  Errors: `400` (amount ≤ 0, bad frequency/date, bad/missing `today`, no lines,
  over-allocated/net-flipped split); `404` (account missing).
- **`DELETE /recurring/:id`** → `204` (lines cascade; generated transactions are **kept**,
  `recurring_id` nulled); `404` if missing.
- **`POST /recurring/post-due`** `{ today: "YYYY-MM-DD" }` → `200 { result: { posted, rules:
  [{ recurringId, posted, error? }] } }`. Generates every due occurrence up to the caller's
  `today` and advances each rule's cursor — **idempotent**; a rule whose split is now invalid
  (e.g. an archived envelope) is reported in `rules[].error` and skipped without blocking the
  others. `400` if `today` is missing/malformed.
...
- **`GET /analysis/budget-vs-actual?month=YYYY-MM`** (`month` **required** — the caller's local
  month, EH8) → `200 { report }`. Errors: `400` (month missing or not `YYYY-MM`).
...
- **`GET /analysis/cash-flow-forecast?accountId=<uuid>&today=YYYY-MM-DD&horizonDays=90&includeExpected=true`** →
  `200 { forecast }`. `today` is **required** — the caller-local day zero the projection starts
  from (EH8). `horizonDays` defaults to **90**, capped `[7, 365]`; `includeExpected` defaults to
