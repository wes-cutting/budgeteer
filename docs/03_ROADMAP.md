...
/accounts/:id/original-principal` → a thin `PayoffView` (sibling of `CreditView`). **One new table**
(`loan_principals`) + the new kind; payoff/owed/trend/roll-up are **derived**. **214 Vitest** (+22: 8
domain + 10 API + 4 web) **+ 1 e2e** (extended with a Payoff step, which also exercises the new
`kind='loan'` end to end); lint + typecheck + format + web build green; verified in a real browser
against the real API. **The analysis area (`#11`–`#14`) is now complete. Next up — hardening: `#15`
backup/export, then `#16` the consolidated a11y/perf/full-e2e pass.**
