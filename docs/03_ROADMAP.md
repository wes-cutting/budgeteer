...
e2e** (extended with a Credit step); lint + typecheck + format + web build green; verified in a real
browser against the real API. **Next up — `#14b` debt payoff %** (the analysis area's last piece), then
...
| 14b | **Analysis — debt payoff %** (installment loans: 1 − owed/original) | slice | Med | Med | an original-principal / loan model (no `kind='loan'` yet) | Planned | The second half of the split `#14`; needs original principal stored + a loan/installment modeling decision (possibly an ADR for effective-dated original principal). Reuses `#14a`'s config-store + trend plumbing |
