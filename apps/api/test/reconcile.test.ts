...
async function makeAccount(startingBalance = "0"): Promise<string> {
  return (await post("/accounts", { name: "Checking", kind: "checking", startingBalance })).json()
...
describe("reconcile-to-bank API (FEAT-010)", () => {
  ...
  test("matched when the entered balance equals the derived balance", async () => {
    ...
    const rec = (
      await post(`/accounts/${accountId}/reconciliations`, { statementBalance: "200.00" })
    ).json().reconciliation;
  ...
  test("accepts a negative statement balance (e.g. a credit account)", async () => {
    ...
    const rec = (
      await post(`/accounts/${accountId}/reconciliations`, { statementBalance: "-125.50" })
    ).json().reconciliation;
  ...
  test("bad amount → 400; missing account → 404", async () => {
    ...
    expect(
      (await post(`/accounts/${accountId}/reconciliations`, { statementBalance: "abc" }))
        .statusCode,
    ).toBe(400);
    expect(
      (await post(`/accounts/${ghost}/reconciliations`, { statementBalance: "10.00" })).statusCode,
    ).toBe(404);
