...
describe("GET /envelopes/:id/ledger (R15)", () => {
  test("empty envelope returns 200 with empty rows", async () => {
    const env = (await post("/api/envelopes", { name: "Groceries" })).json().envelope;
    const res = await get(`/api/envelopes/${env.id}/ledger`);
  ...
  test("rows newest-first; deposit amount positive, withdrawal amount negative", async () => {
    ...
    await post(`/api/accounts/${acct.id}/transactions`, {
      kind: "withdrawal",
      amount: "100.00",
    ...
    const { rows } = (await get(`/api/envelopes/${env.id}/ledger`)).json();
  ...
  test("opening-kind rows are included", async () => {
    ...
    const env = (await post("/api/envelopes", { name: "Emergency" })).json().envelope;

    const txns = (
      await get(`/api/accounts/${acct.id}/transactions?from=2026-07-01&to=2026-07-31`)
    ).json().transactions;
  ...
  test("ledger only returns allocations for the requested envelope", async () => {
    ...
    const env1 = (await post("/api/envelopes", { name: "Groceries" })).json().envelope;
    const env2 = (await post("/api/envelopes", { name: "Gas" })).json().envelope;
    ...
    const rows1 = (await get(`/api/envelopes/${env1.id}/ledger`)).json().rows;
    const rows2 = (await get(`/api/envelopes/${env2.id}/ledger`)).json().rows;
  ...
  test("missing or wrong-household envelope id → 404", async () => {
    ...
    const res = await get(`/api/envelopes/${ghost}/ledger`);
